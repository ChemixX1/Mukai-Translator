...
def validate_translator(main: ComicTranslate, target_lang: str):
    """Ensure translator is selected and API credentials are configured."""
    settings_page = main.settings_page
    ...
    # Custom requires api_key, api_url, and model
    ...
    # All other LLM providers require an api_key
    llm_service_map = {
        'GPT':      tr('Open AI GPT'),
        'Claude':   tr('Anthropic Claude'),
        'Gemini':   tr('Google Gemini'),
        'Deepseek': tr('Deepseek'),
        'Grok':     tr('Grok'),
    }
    for key, service in llm_service_map.items():
        if key in translator_tool:
            creds = credentials.get(service, {})
            if not creds.get('api_key'):
                Messages.show_api_key_required_error(main, service)
                return False
            break
